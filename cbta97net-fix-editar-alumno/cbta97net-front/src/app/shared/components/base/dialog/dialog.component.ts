...
import type { ZardIcon } from '@/shared/components/base/icon/icons';
  ...
  template: `
    ...
    @if (config.zTitle || config.zDescription) {
      <header class="flex flex-col space-y-1.5 text-center sm:text-left">
        @if (config.zTitle) {
          <h4 data-testid="z-title" class="text-xl leading-none font-semibold tracking-tight">{{ config.zTitle }}</h4>
