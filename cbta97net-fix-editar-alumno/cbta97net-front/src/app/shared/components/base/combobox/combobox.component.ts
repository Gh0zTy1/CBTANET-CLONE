...
import { ZardButtonComponent, ZardButtonSizeVariants, type ZardButtonTypeVariants } from '@/shared/components/base/button';
import { comboboxVariants, ZardComboboxSizeVariants, type ZardComboboxWidthVariants } from '@/shared/components/base/combobox/combobox.variants';
import {
...
import { ZardEmptyComponent } from '@/shared/components/base/empty';
import { ZardIconComponent, type ZardIcon } from '@/shared/components/base/icon';
import { ZardPopoverComponent, ZardPopoverDirective } from '@/shared/components/base/popover';
  ...
  template: `
    ...
    >
      @if(placeholder() && !displayValue()) {
        <span class="flex-1 truncate text-left font-normal px-1 text-gray-500" [class]="labelClasses()">
          {{ placeholder() }}
        </span>
      }

      @if(displayValue()) {
        <span class="flex-1 truncate text-left font-normal px-1 text-slate-600" [class]="labelClasses()">
          {{ displayValue() }}
        </span>
      }
...
export class ZardComboboxComponent implements ControlValueAccessor {
  ...
  readonly buttonSize = input<ZardButtonSizeVariants>('default');
  ...
  protected labelClasses = computed(() => {
    if(this.buttonSize() === "sm") "text-(length:--text-xs)"
    if(this.buttonSize() === "default") "text-(length:--text-sm)"
    if(this.buttonSize() === "lg") "text-(length:--text-sm)"
  })
